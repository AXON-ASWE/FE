'use client';

import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import Image from 'next/image';
import { useEffect, useState } from 'react';
import { Pencil, Save, X } from 'lucide-react';

type AdminProfileDTO = {
  fullName: string;
  email: string;
  phone: string;
  gender: string;
  dateOfBirth: string;
  status: string;
  createdAt: string;
  image: string;
};

export default function AdminProfilePage() {
  const [data, setData] = useState<AdminProfileDTO | null>(null);

  const [editingField, setEditingField] = useState<string | null>(null);
  const [tempValue, setTempValue] = useState<string>('');

  useEffect(() => {
    const res: AdminProfileDTO = {
      fullName: 'Admin Nguyễn Văn A',
      email: 'admin@example.com',
      phone: '0901234567',
      gender: 'Nam',
      dateOfBirth: '1990-05-20',
      status: 'Active',
      createdAt: '2022-11-10',
      image:
        'https://images.pexels.com/photos/5215024/pexels-photo-5215024.jpeg?auto=compress&cs=tinysrgb&w=200&h=200&fit=crop',
    };
    setData(res);
  }, []);

  if (!data) return <div>Đang tải...</div>;

  const startEdit = (field: keyof AdminProfileDTO) => {
    setEditingField(field);
    setTempValue(data[field]);
  };

  const saveEdit = () => {
    if (!editingField || !data) return;

    setData({
      ...data,
      [editingField]: tempValue,
    });

    setEditingField(null);
  };

  const cancelEdit = () => {
    setEditingField(null);
  };

  const InfoRow = ({
    label,
    field,
  }: {
    label: string;
    field: keyof AdminProfileDTO;
  }) => {
    const isEditing = editingField === field;

    return (
      <div className="relative group flex flex-col">
        <p className="text-gray-500">{label}</p>

        {isEditing ? (
          <div className="flex items-center gap-2 mt-1">
            <input
              value={tempValue}
              onChange={(e) => setTempValue(e.target.value)}
              className="border px-2 py-1 rounded w-60"
            />
            <button onClick={saveEdit} className="text-green-600">
              <Save size={18} />
            </button>
            <button onClick={cancelEdit} className="text-red-500">
              <X size={18} />
            </button>
          </div>
        ) : (
          <div className="flex items-center gap-2">
            <p className="font-medium">{data[field]}</p>
            <button
              onClick={() => startEdit(field)}
              className="opacity-0 group-hover:opacity-100 transition text-gray-400 hover:text-blue-600"
            >
              <Pencil size={16} />
            </button>
          </div>
        )}
      </div>
    );
  };

  return (
    <div className="w-full space-y-6">
      <Card className="shadow-sm border border-gray-200">
        <CardHeader>
          <CardTitle className="text-lg">Thông tin cá nhân</CardTitle>
        </CardHeader>

        <CardContent>
          <div className="flex gap-6 items-start">
            <Image
              src={data.image}
              width={160}
              height={160}
              alt="Admin Avatar"
              className="rounded-xl object-cover"
            />

            {/*  2 columns */}
            <div className="grid grid-cols-2 gap-x-12 gap-y-6">
              <InfoRow label="Họ và tên" field="fullName" />
              <InfoRow label="Email" field="email" />
              <InfoRow label="Số điện thoại" field="phone" />
              <InfoRow label="Giới tính" field="gender" />
              <InfoRow label="Ngày sinh" field="dateOfBirth" />
              <InfoRow label="Trạng thái" field="status" />
              <InfoRow label="Ngày tạo" field="createdAt" />
            </div>
          </div>
        </CardContent>
      </Card>
    </div>
  );
}
